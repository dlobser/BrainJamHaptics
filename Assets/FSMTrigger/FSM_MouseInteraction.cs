...
public class FSM_MouseInteraction : MonoBehaviour {
	...
	void Update() {
		...
		if (UseMouse)
            ...
            hitsInfo = Physics.RaycastAll(new Ray(objPos.transform.position, objPos.transform.forward));
		}
		...
		if (hit) {
			...
			if (hitInfo.transform.gameObject.GetComponent<ON.ON_FSM> () != null) {
                ...
                foreach(RaycastHit h in hitsInfo)
                    if(h.transform.gameObject.GetComponent<ON.ON_FSM>()!=null)
                        h.transform.gameObject.GetComponent<ON.ON_FSM>().Ping();
			}
