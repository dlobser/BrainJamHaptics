...
//			GetComponent<ON_InteractableEvents> ().OnIdle -= Idle;
		...
		void SetMoveToObject(){
			if (moveToFirstObject && localPosition)
				move = val.objectsToModify [0].transform.localPosition;
			else if(moveToFirstObject)
				move = val.objectsToModify [0].transform.position;

//			Debug.Log ("move: " + move);
//			Debug.Log ("Position : " + val.objectsToModify [0].transform.position);
//			Debug.Log ("LocalPosition : " + val.objectsToModify [0].transform.localPosition);
		}
